...
export default function Reports() {
  const { donations, expenses, loans, repayments, surplus } = useData();
  const currentYear = new Date().getFullYear();
  const [year, setYear] = useState(currentYear);
  const [donorName, setDonorName] = useState('');
  const [donorYear, setDonorYear] = useState(currentYear);

  const allDates = [...donations, ...expenses, ...loans, ...repayments, ...surplus].map((x) => new Date(x.date).getFullYear());
  ...
  let gDon = 0, gExp = 0, gLoan = 0, gRep = 0, gSur = 0;
  const monthlyRows = MONTH_NAMES.map((month, m) => {
    ...
    const sur = surplus.filter((s) => inMY(s.date, m, year)).reduce((s2, s) => s2 + Number(s.amount), 0);
    gDon += don; gExp += exp; gLoan += loan; gRep += rep; gSur += sur;
    return { month, don, exp, loan, rep, sur, net: don - exp - loan + rep + sur };
  });
    ...
    <div className="space-y-6">
      <Card>
        ...
        <CardBody>
          ...
          <Table>
            <Thead columns={['Month', 'Donations', 'Expenses', 'Loans Given', 'Loan Repaid', 'Surplus', 'Net Balance']} />
            <tbody>
              {monthlyRows.map((r, i) => {
                const has = r.don || r.exp || r.loan || r.rep || r.sur;
                return (
                  <Tr key={i} className={!has ? 'text-gray-400' : ''}>
                    ...
                    <Td className={r.sur > 0 ? 'amount-positive' : ''}>{formatPKR(r.sur)}</Td>
            ...
            <tfoot>
              <tr className="bg-gray-100 font-semibold border-t-2 border-dark">
                ...
                <Td>{formatPKR(gSur)}</Td>
                <Td>{formatPKR(gDon - gExp - gLoan + gRep + gSur)}</Td>
              </tr>
            </tfoot>
          </Table>
        </CardBody>
